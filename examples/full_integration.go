...
func main() {
	...
	for i := 0; i < 3; i++ {
		...
		blocksGauge := metricsCollector.CreateCounter("blocks_added", nil)
		blocksGauge.Increment()
	...
	perfLogger.LogGeneralPerformance("full_integration_demo", time.Since(time.Now()))

	// Логируем использование памяти
	perfLogger.LogMemoryUsage()

	// Логируем количество горутин
	perfLogger.LogGoroutineCount()
